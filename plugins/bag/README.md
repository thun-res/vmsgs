...
`vmsgs-bag-plugin` is a VLink `BagPluginInterface` 2.0 plugin. On the write path it extracts the measurement timestamp from vmsgs Protobuf and FlatBuffers messages, or from VLink ZeroCopy messages, and reorders frames through `BagProcessor`. The read path forwards frames unchanged.
...
vlink-bag record --plugin vmsgs-bag-plugin ...
```
...
| `VMSGS_BAG_RESORT` | `0` | Set to `1` to enable data-timestamp reordering. |
